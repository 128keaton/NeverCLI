    ...
    int Streamer::start() {
        ...
        if (this->type == h265) {
            ...
            if (!this->has_vaapi) {
                ...
                g_object_set(G_OBJECT(appData.encoder), "max-bframes", 5, nullptr); // max-bframes (not i-frames)
                g_object_set(G_OBJECT(appData.encoder), "keyframe-period", 60, nullptr); // auto (duh)
