    ...
    void Streamer::setupStreamOutput(StreamData *appData, bool create_encoder) {
        ...
        if (!has_vaapi && !has_nvidia) {
            ...
            if (appData->is_h265) {
                ...
                g_object_set(G_OBJECT(appData->decoder), "min-force-key-unit-interval", 30000000000, nullptr);
            } else {
            ...
            if (create_encoder) {
               ...
               g_object_set(G_OBJECT(appData->encoder), "min-force-key-unit-interval", 30000000000, nullptr);
